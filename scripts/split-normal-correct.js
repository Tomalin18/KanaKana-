...
function splitNormalVocabulary() {
  console.log('🔄 開始分割 normal 2.ts 詞彙檔案，參考 expert 分割格式...');

  const inputFile = path.join(__dirname, '../src/data/vocabulary-final/normal 2.ts');
  const outputDir = path.join(__dirname, '../src/data/vocabulary-final/normal-split');
  ...
  try {
    ...
    const arrayStartIndex = lines.findIndex(line => line.includes('export const NORMAL_WORDS: TetrisWord[] = ['));
...
// Import all chunks
${Array.from({ length: chunks }, (_, i) => `import { NORMAL_CHUNK_${i + 1} } from './normal-chunk-${i + 1}';`).join('\n')}

// Combine all chunks to maintain the same interface
export const NORMAL_WORDS: TetrisWord[] = [
${Array.from({ length: chunks }, (_, i) => `  ...NORMAL_CHUNK_${i + 1}`).join(',\n')}
];`;
...
/**
 * NORMAL_CHUNK_${i + 1} - Normal Vocabulary Chunk ${i + 1}
 * 
 * 🔄 生成時間: ${new Date().toISOString()}
...
export const NORMAL_CHUNK_${i + 1}: TetrisWord[] = [
${chunkItems.map((item, index) => {
  ...
  return item + ',';
      ...
      fs.writeFileSync(path.join(outputDir, `normal-chunk-${i + 1}.ts`), chunkContent, 'utf8');
