...
function checkKanaIssues(filePath) {
  ...
  for (const line of lines) {
    ...
    if (line.includes('"kana":') || line.includes('kana:')) {
      ...
      if (kanaMatch) {
        ...
        // 檢查各種問題模式
        for (const [patternName, pattern] of Object.entries(KANA_PROBLEM_PATTERNS)) {
          if (pattern.test(kanaValue)) {
            const matches = kanaValue.match(pattern);
            if (matches) {
              issues.push({
                type: patternName,
                line: lineNumber,
  ...
  for (const result of allResults) {
    ...
    if (result.issues.length === 0) {
      ...
      for (const issue of result.issues) {
        ...
        console.log(`   ${issueType}: 第${issue.line}行 - "${issue.kana}"`);
      }
    }
  }
  ...
  if (totalIssues > 0) {
    ...
    for (const result of allResults) {
      if (result.issues.length > 0) {
        console.log(`\n📁 ${result.file}:`);
        for (const issue of result.issues) {
          console.log(`  行 ${issue.line}: ${issue.type} - "${issue.kana}"`);
        }
      }
    }
  }
...
function main() {
  console.log('🚀 開始檢查所有詞彙文件的 kana 標點符號問題...');
  ...
  for (const fileName of VOCABULARY_FILES) {
    ...
    const result = checkKanaIssues(filePath);
  ...
  // 保存報告到文件
  const reportPath = path.join(__dirname, 'kana-issues-report.json');
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  console.log(`\n📄 詳細報告已保存到: ${reportPath}`);

  if (report.totalIssues === 0) {
    console.log('\n🎉 恭喜！所有 kana 字段都沒有標點符號問題！');
  } else {
    console.log(`\n⚠️  發現 ${report.totalIssues} 個 kana 標點符號問題需要修復`);
  }
}
...
module.exports = { checkKanaIssues, generateReport }; 